...
for sub=1:n_subs
    ...
    if rsav.lss
        a          = load(fullfile(ana_dir,sprintf('lss_RDMi_%s.mat',rsav.atlas)));
    else
...
for gg=1:numel(pruned_roi_ind)
    ...
    for c_i = 1:numel(rsav.models)
        if size(rsav.models{c_i}.RDM,1) > 1
            c_model_vec = squareform(rsav.models{c_i}.RDM);
        else
            c_model_vec = rsav.models{c_i}.RDM;
        end
...
for g = 1:numel(rsav.models)    
    new_image_V.fname = fullfile(path.secondlevelDir,rsav.ana_name,sprintf('%s%s_%s',descr, rsav.models{g}.name,'-logP.nii'));    
