...
% function [u_hat,Sw_hat,resMS,beta_hat]=noiseNormalizeBeta(Y,SPM,varargin)
...
switch (Opt.normmode)
    case 'runwise'              % do run-wise noise normalization
        ...
        for i=1:Nrun
            ...
            [Sw_reg(:,:,i),shrinkage(i),Sw_hat(:,:,i)]=covdiag(res(idxT,:),SPM.xX.trRV/(Nrun*mean(diag(SPM.xX.Bcov))),'shrinkage',Opt.shrinkage);                    %%% regularize Sw_hat through optimal shrinkage
    ...
    case 'overall'              %%% do overall noise normalization
        % in the scaling of the noise, take into account mean beta-variance 
        [Sw_reg,shrinkage,Sw_hat]=covdiag(res,SPM.xX.trRV/mean(diag(SPM.xX.Bcov)),'shrinkage',Opt.shrinkage);   %%% regularize Sw_hat through optimal shrinkage
