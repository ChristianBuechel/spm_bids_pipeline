...
if isempty(Nback)
    ...
    %XYZvox           = inv(Vnormalized.mat)*[XYZmm; ones(1,size(XYZmm,2))]; % get XYZ in voxel coordinates 
    XYZvox           = inv(Nback.mat)*[XYZmm; ones(1,size(XYZmm,2))]; % The above was wrong, but it worked for nonlin coreg bc the .mat were the same!
    XYZvox           = XYZvox(1:3,:); % remove ones
