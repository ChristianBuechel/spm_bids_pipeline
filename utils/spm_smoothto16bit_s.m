function VO = spm_smoothto16bit_s(V,fwhm)
...
%_______________________________________________________________________
function VO = smoothto16bit_s(V,fwhm)
...
for vols = 1:size(V,1)
    ...
    VO(vols).dt     = [spm_type('int16') spm_platform('bigend')];
    VO(vols).pinfo  = [];
    ...
    V0(vols).dat    = int16(0);
    V0(vols).dat(VO(vols).dim(1:3)) = int16(0);
    ...
    for i=1:V(vols).dim(3)+r{3}.s,
        ...
        if i>r{3}.s,
            ...
            VO(vols).pinfo(1:2,ii) = [(mx-mn)/32767 mn]';
            VO(vols).dat(:,:,ii)   = int16(round((img-mn)*(32767/(mx-mn))));
        end
    end
end
